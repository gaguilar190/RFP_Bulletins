...
if "requirements_json" not in st.session_state:
    st.session_state["requirements_json"] = json.dumps(
...
if run_button:
    ...
    try:
        ...
        raw_requirements = apply_target_profiles(
            raw_requirements,
            brief_text,
        )
    ...
    with st.spinner("Scoring proposal candidates..."):
        ...
        excluded = inventory[
            ~inventory.index.isin(selected.index)
        ].copy()

    st.session_state["run_completed"] = True

    st.success(
        f"Selected {len(selected)} units. Excluded {len(excluded)} units."
    )
    ...
    with st.spinner("Writing output workbook..."):
        ...
        if template_file is not None:
            ...
            suffix = (
                ".xlsm"
                if template_file.name.lower().endswith(".xlsm")
                else ".xlsx"
            )
